...
add_action( 'wp_enqueue_scripts', function() {
	wp_register_script( 'ajax-filter', get_template_directory_uri() . '/blocks/filter-block/filter.js', ['jquery'], time(), true );
	wp_localize_script( 
        'ajax-filter',
        'ajax_filter_params',
		array(
...
add_action( 'wp_ajax_handle_ajax_filter', 'handle_ajax_filter' );
add_action( 'wp_ajax_nopriv_handle_ajax_filter', 'handle_ajax_filter' );
...
function handle_ajax_filter() {
    ...
    if($_POST['cat'] != 0) :
        ...
        $transient_key_cat = TRANSIENT_PREFIX . "_" . $_POST['cat'] . "_posts";
    elseif($_POST['industry'] != 0) :
        $tax_query = [
            [
                'taxonomy' => 'resource-industry',
                'field'    => 'term_id',
                'terms'    => $_POST['industry'],
            ]
        ];
        $transient_key_cat = TRANSIENT_PREFIX . "_" . $_POST['industry'] . "_posts";
    else :
        $tax_query = [];
        $transient_key_cat = "";
    endif;

    if($transient_key_cat) {
        $custom_query = get_transient($transient_key_cat);
    }
    ...
    if(!$custom_query) {      
        ...
        set_transient($transient_key_cat, $custom_query, TRANSIENT_DURATION);
    }
                                ...
                                <div class="flex ~gap-[0.5rem]/[1rem] flex-wrap items-center justify-start">
                                    <?php 
                                        ...
                                        foreach ( $industries as $industry ) {
                                            echo '<div class="bg-[#E09762] text-black px-[0.75rem] py-[0.25rem] font-mono text-[0.75rem] rounded-[1.5rem] text-center lowercase">' . $industry->name . '</div>' . ' ';
                                        } 
                                    ?>
                                </div>
    ...
    <?php else : ?>
        ...
        echo json_encode( array(
            'posts'      => '<div class="text-[1rem]">No posts found.</div>',
            'pagination' => '',
        ) ); ?>
    <?php endif;
